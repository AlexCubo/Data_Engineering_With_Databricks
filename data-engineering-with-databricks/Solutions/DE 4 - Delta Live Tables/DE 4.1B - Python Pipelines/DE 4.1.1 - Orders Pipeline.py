...
# MAGIC * Describing differences in syntax and execution of Materialized View and streaming tables
...
# MAGIC * **Materialized Views** are materialized views for the lakehouse; they will return the current results of any query with each refresh
# MAGIC * **Streaming tables** are designed for incremental, near-real time data processing
# MAGIC
# MAGIC Note that both of these objects are persisted as tables stored with the Delta Lake protocol (providing ACID transactions, versioning, and many other benefits). We'll talk more about the differences between Materialized View and streaming tables later in the notebook.
# MAGIC
...
# MAGIC
# MAGIC The two functions we've reviewed so far have both created streaming tables. Below, we see a simple function that returns a live table (or materialized view) of some aggregated data.
# MAGIC
...
# MAGIC * Enforcing data quality with constraints
# MAGIC * Adding comments to tables
# MAGIC * Describing differences in syntax and execution of live tables streaming tables
# MAGIC
