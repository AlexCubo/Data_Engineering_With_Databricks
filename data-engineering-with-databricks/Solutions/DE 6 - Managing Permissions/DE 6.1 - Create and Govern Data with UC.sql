...
-- MAGIC Back in the same query in the Databricks SQL session, let's replace *gold* with *silver* and run the query. As the data owner, running this should return all table results as expected. However, if you were to run this as another user, this would fail because we never set up permissions on the *silver* table. 
-- MAGIC
-- MAGIC Querying *gold* works for all members of the _account users_ group because the query represented by a view is essentially executed as the owner of the view. This important property enables some interesting security use cases; in this way, views can provide users with a restricted view of sensitive data, without providing access to the underlying data itself. We will see more of this shortly.
-- MAGIC
...
-- MAGIC ## Revoke access
-- MAGIC
-- MAGIC No data governance platform would be complete without the ability to revoke previously issued grants. Let's start by examining access to the *my_mask()* function.
