...
-- MAGIC
-- MAGIC Let's create and populate a simple collection of schemas and tables pursuant to the Delta architecture:
...
-- MAGIC %md
-- MAGIC This command works for us since we are the owner of the view. However, it will not work yet for other members of the **account users** group because **SELECT** privilege on the table alone is insufficient. **USAGE** privilege is also required on the containing elements. Let's correct this now by executing the following.
